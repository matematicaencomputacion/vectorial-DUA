...
**Instrumentación que sostiene estos umbrales (pendiente, PR chico):** `go run ./cmd/harness -suite bench` — benchmarks de `Nearest` y `Retrieve` a 100 / 1.000 / 10.000 / 100.000 nodos sintéticos, corriendo en CI, para que el cruce de umbral sea un aviso automático y no un descubrimiento en producción.
